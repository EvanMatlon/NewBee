var TableEditable = function () {
    ...
    var handleTable = function () {
        ...
        function editRow(oTable, nRow) {
            ...
            jQuery.get("/api/exe_group",
	            function(data){
		      	   $.each(data.results, function(id, value){
		      	   $("#groupinfo").append("<option value=" + value['id'] + ">" +    value['exe_groupname'] +"</option>");
		      	   });
           ...
           $("#roleinfo").change(function(){
               role_select=$("#roleinfo").children("option:selected").text()
               role_value=$("#roleinfo").children("option:selected").val()
               group_value=$("#groupinfo").children("option:selected").text()
               if(group_value=='---请选择---'){
                   alert('请先选择执行组<!0_ _0!>')
                   return;
               }
               if(role_select!='---请选择---'){
                   jQuery.get("/api/role/?id="+role_value,
                         function(data){
                             $.each(data.results, function(id, value){
                                ...
                                for (var i=0;i<arr_vars.length;i++){
                                  str+= '<p>'+ trim(arr_vars[i])+ ':<input name='+ trim(arr_vars[i]) + ' type="text" class="input" size="20"></p>'
                                }
                    ...
                    $("#transdb_button").click(function(){
                               ...
                               if (i!=0){
                                   ...
                                   var xhr;
                                    ...
                                    url='/api/exe_group/'+$("#groupinfo").children("option:selected").val()+'/';
        ...
        function editsaveRow(oTable, nRow) {
           ...
           var data = {};
            ...
            url='http://127.0.0.1:8000/api/mission/'+jqInputs[1].value+'/';
            console.info(url);
...
function search() {
    ...
    window.location.href="../finished_mission?exe_groupname="+a
}
